...
export const QuantityController = ({ value, onChange, min = 0, max = 99, bgBtn = 'brand-light'}: QuantityControllerProps) => {
   ...
   return (
      <div className="flex items-center justify-between min-h-[35px] min-w-[80px] py-[4px] ">
         <button
            onClick={() => onChange(Math.max(min, value - 1))}
            disabled={value <= min}
            className={`${buttonBaseClass} ${showBackground ? `bg-${bgBtn} `: 'border-[2px] border-neutral-borderLight'
               }`}
         >
         ...
         <button
            ...
            disabled={value >= max}
            className={`${buttonBaseClass} ${value < max ? `bg-${bgBtn}` : 'border-[2px] border-neutral-borderLight'}`}
         >
